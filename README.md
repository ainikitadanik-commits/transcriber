...
По умолчанию применяется локальная ad-hoc подпись. Для внешнего релиза укажите
сертификат Developer ID:

```bash
SIGN_IDENTITY="Developer ID Application: ..." scripts/build_app_bundle.sh
```

После этого релиз всё равно требуется нотариализовать и прикрепить билет Apple.
Постоянный bundle ID продукта: `com.ainikitadanik.transcriber`.

## Сборка пилотного DMG
