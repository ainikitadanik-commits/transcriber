...
На текущем этапе помощник только проверяет оба локальных аудиопотока и ничего
не записывает на диск; передача звука в GigaAM будет добавлена следующим этапом.
