...
- near-real-time распознавание системного звука с акустическим разделением
  участников на анонимные метки `Спикер 1`, `Спикер 2`, …;
- аварийное восстановление committed-текста незавершённой realtime-встречи;
- надёжная работа длинных realtime-сессий с инкрементальным отображением текста;
...
В установленной `.app` интерфейс открывается в отдельном окне Транскрибатора, а
локальный адрес `http://127.0.0.1:7860` не показывается в браузере. Загруженные
файлы сохраняются в `input`, результаты — в `output`. При техническом запуске
из Terminal интерфейс остаётся доступен только на этом Mac по тому же адресу.
...
диск. Накопленные окна распознаются локально GigaAM. При включённом разделении
участников pyannote анализирует тот же системный PCM параллельно, а временные
метки слов связываются с анонимными голосами. Имена или каналы из сервиса ВКС
не запрашиваются.
...
Такой файл содержит `INTERNAL` в имени, не является публичным
